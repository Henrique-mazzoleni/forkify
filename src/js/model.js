  ...
  try {
    const ingredients = Object.entries(newRecipe)
      ...
      .map((ing) => {
        ...
        if (ingArr.length !== 3)
          throw new Error(
            "Input the Ingredients in the right format, you dumbass."
          );
...
export const removeBookmark = function (recipe) {
  if (recipe.id === state.recipe.id && !state.recipe.bookmarked) {
    console.log(state.recipe, state.bookmarks);
    return;
  }
  state.recipe.bookmarked = false;
  const index = state.bookmarks.findIndex((bkm) => bkm.id === recipe.id);
